...
# Title generation
def generate_title(platform, topic, tone="engaging",  seo=False, include_emojis=False, title_char_limit=120, title_word_limit=15, num_suggestions=5):
 
    if seo:
        prompt = (
            f"Act as an SEO expert and a {platform} marketer. Create an SEO-optimized {platform} title for {topic}. "
            f"The title should be {tone} and optimized within {title_char_limit} characters and {title_word_limit} words. "
        )
    else:
        prompt = (
            f"Act as a {platform} marketer. Create a {platform} title for {topic}. "
            f"The title should be {tone} and optimized within {title_char_limit} characters and {title_word_limit} words. "
        )
    ...
    main_title = titles[0] # these are input fields
